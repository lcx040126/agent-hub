...
function managedArgs(connectionId: string): string[] {
  return ["runner.js", "--mcp-bridge", "--connection-id", connectionId];
}
...
describe("removeCodexMcpConfig", () => {
  ...
  it("removes only the four exact managed hook handlers with the last managed server", () => {
    const source = mergeCodexMcpConfig([
      "[[hooks.PreToolUse]]",
    ...
    expect(allHandlers.map((handler) => handler.command)).toEqual(
      expect.arrayContaining([
        ...
        `${MANAGED_HOOK_COMMAND} --codex-hook SessionEnd`,
      ]),
    );
  });
});
