...
export type { PauseMemberInput, PauseMemberResult } from "./domain.js";
...
interface PauseMemberHistory {
  result: PauseMemberResult;
...
export class AgentHubService {
  ...
  /**
   * Pause the authenticated member's integration and clean up work visible on
   * first handling using the room server clock. The operation is intentionally
   * idempotent: a retry with the same request id returns the original result,
   * while a reused id with different arguments is rejected.
  */
  pauseMember(input: PauseMemberInput): PauseMemberResult {
    return this.database.transaction(() => {
      const auth = this.authenticateMemberToken(input.memberToken);
      const requestId = requiredString(input.requestId, "Pause request id", 128);
      const reason = requiredString(input.reason, "Pause reason", 2_000);
      const requestedCutoffAt = normalizePauseCutoff(input.cutoffAt);
      const now = this.timestamp();
      // Session and lease timestamps are generated by this room server, so the
      // applied boundary must use the same clock domain. Clients block new
      // remote work while an offline pause is queued.
      const cutoffAt = now;
      const previous = this.findPauseResult(auth.room.id, auth.member.id, requestId);
      if (previous) {
        if (
          previous.result.reason !== reason
          || previous.requestedCutoffAt !== requestedCutoffAt
        ) {
          throw new AgentHubError(
            "pause_request_conflict",
            "The pause request id was already used with different arguments.",
            409,
            { requestId },
          );
        }
      ...
      const sessionRows = this.database.connection
        .prepare(`
          SELECT id
          FROM work_sessions
        ...
        `)
        .all(auth.room.id, auth.member.id, cutoffAt) as Row[];
      const closedSessionIds = sessionRows.map((row) => asString(row.id));

      const confirmationRows = this.database.connection
        .prepare(`
          SELECT confirmations.id, confirmations.session_id
          FROM feature_change_confirmations confirmations
          JOIN work_sessions sessions ON sessions.id = confirmations.session_id
          WHERE confirmations.member_id = ?
            AND confirmations.status IN ('pending', 'approved')
            AND sessions.room_id = ? AND sessions.member_id = ?
            AND sessions.opened_at <= ?
          ORDER BY confirmations.created_at ASC, confirmations.id ASC
        `)
        .all(auth.member.id, auth.room.id, auth.member.id, cutoffAt) as Row[];
      const expiredConfirmationIds = confirmationRows.map((row) => asString(row.id));
      const confirmationSessionIds = uniqueStrings(
        confirmationRows.map((row) => asString(row.session_id)),
      );

      const leaseRows = this.database.connection
        .prepare(`
          ...
          WHERE room_id = ? AND member_id = ? AND status = 'active'
            AND (
              ...
              OR (
                ...
                AND EXISTS (
                  SELECT 1
                  FROM work_sessions session_scope
                  WHERE session_scope.id = leases.session_id
                    AND session_scope.room_id = ?
                    AND session_scope.member_id = ?
                    AND session_scope.opened_at <= ?
                )
              )
            )
      ...
      const releasedLeaseIds = leaseRows.map((row) => asString(row.id));

      // Only requests that existed at the cutoff are cancelled, except a
      // request targeting a lease released by this operation. The latter may
      // have been created just after the cutoff and is still directly related
      // to the cleanup.
      const candidateReleaseRequestRows = this.database.connection
        .prepare(`
          SELECT DISTINCT rr.id, rr.requested_at,
            rr.requester_member_id, rr.conflicting_lease_id,
            conflicting.member_id AS conflicting_member_id
          FROM release_requests rr
          LEFT JOIN leases conflicting ON conflicting.id = rr.conflicting_lease_id
          WHERE rr.room_id = ? AND rr.status = 'pending'
          ORDER BY rr.requested_at ASC, rr.id ASC
        `)
        .all(auth.room.id) as Row[];
      const releasedLeaseSet = new Set(releasedLeaseIds);
      const releaseRequestRows = candidateReleaseRequestRows.filter((row) => {
        if (releasedLeaseSet.has(asString(row.conflicting_lease_id))) return true;
        const requestedAt = Date.parse(asString(row.requested_at));
        return Number.isFinite(requestedAt)
          && requestedAt <= Date.parse(cutoffAt)
          && (
            asString(row.requester_member_id) === auth.member.id
            || asString(row.conflicting_member_id) === auth.member.id
          );
      });
      const cancelledReleaseRequestIds = releaseRequestRows.map((row) => asString(row.id));

      const closedAt = now;
      for (const sessionId of confirmationSessionIds) {
        this.featureMemory.expireSessionConfirmations(sessionId, closedAt);
      }
      for (const sessionId of closedSessionIds) {
        this.database.connection
          .prepare(`
            UPDATE work_sessions
            SET status = 'closed', frozen_reason = NULL, closed_at = ?, last_seen_at = ?
            WHERE id = ? AND room_id = ? AND member_id = ?
              AND status IN ('active', 'frozen') AND opened_at <= ?
          `)
          .run(closedAt, closedAt, sessionId, auth.room.id, auth.member.id, cutoffAt);
        this.insertActivity({
          roomId: auth.room.id,
          ...
          entityType: "session",
          entityId: sessionId,
          summary: `${auth.member.displayName} closed a session when Agent Hub integration paused.`,
          metadata: { memberPause: true, requestId },
          createdAt: closedAt,
        });
      }
      ...
      const cancelLease = this.database.connection.prepare(`
        ...
        WHERE id = ? AND room_id = ? AND member_id = ?
          AND status = 'active'
      `);
      const cancellationSummary = `Cancelled because Agent Hub integration paused: ${reason}`;
      for (const lease of leaseRows) {
        ...
        this.insertActivity({
          ...
          entityType: "lease",
          entityId: leaseId,
          summary: `${auth.member.displayName} cancelled ${asString(lease.title)} when Agent Hub integration paused.`,
          metadata: {
            memberPause: true,
            requestId,
            sessionId: nullableString(lease.session_id),
            status: "cancelled",
          },
          createdAt: closedAt,
        });
      ...
      `);
      for (const request of releaseRequestRows) {
        cancelReleaseRequest.run(
          auth.member.id,
          closedAt,
          asString(request.id),
          auth.room.id,
      ...
      const result: PauseMemberResult = {
        requestId,
        roomId: auth.room.id,
        ...
        expiredConfirmationCount: expiredConfirmationIds.length,
      };
      this.auditRecord(
        auth,
        "member.paused",
        "member",
        auth.member.id,
        `${auth.member.displayName} paused Agent Hub integration.`,
        { ...result, requestedCutoffAt },
      );
      return result;
    });
  }
  ...
  private findPauseResult(
    ...
    memberId: string,
    requestId: string,
  ): PauseMemberHistory | null {
    const rows = this.database.connection
      .prepare(`
        SELECT *
        FROM activities
        WHERE room_id = ? AND actor_member_id = ? AND type = 'member.paused'
        ORDER BY created_at DESC, id DESC
      `)
      .all(roomId, memberId) as Row[];
    for (const row of rows) {
      const metadata = parseObject(row.metadata_json);
      if (metadata.requestId !== requestId) continue;
      const reason = typeof metadata.reason === "string" ? metadata.reason : "";
      const cutoffAt = typeof metadata.cutoffAt === "string" ? metadata.cutoffAt : "";
      const requestedCutoffAt = typeof metadata.requestedCutoffAt === "string"
        ? metadata.requestedCutoffAt
        : cutoffAt;
      const appliedAt = typeof metadata.appliedAt === "string"
        ? metadata.appliedAt
        : asString(row.created_at);
      const closedSessionIds = metadataStringArray(metadata.closedSessionIds);
      const releasedLeaseIds = metadataStringArray(metadata.releasedLeaseIds);
      const cancelledReleaseRequestIds = metadataStringArray(metadata.cancelledReleaseRequestIds);
      const expiredConfirmationIds = metadataStringArray(metadata.expiredConfirmationIds);
      const memberRole = metadata.memberRole === undefined
        ? "member"
        : metadata.memberRole;
      if (memberRole !== "host" && memberRole !== "member") {
        throw new AgentHubError(
          "pause_history_corrupt",
          "The existing pause request member role is invalid.",
          500,
          { requestId },
        );
      }
      if (!reason || !cutoffAt || !appliedAt) {
        throw new AgentHubError(
          "pause_history_corrupt",
          "The existing pause request record is incomplete.",
          500,
          { requestId },
        );
      }
      return {
        ...
        result: {
          ...
          roomId,
          memberId,
          // Pre-v0.2.1 development records did not persist the role. Treat
          // those as members so an old retry can never stop a shared room.
          memberRole,
          reason,
          cutoffAt,
          appliedAt,
...
function normalizePauseCutoff(value: unknown): string {
  const input = requiredString(value, "Pause cutoffAt", 80);
  const timestamp = Date.parse(input);
  if (!Number.isFinite(timestamp)) {
    throw new AgentHubError("invalid_pause_cutoff", "Pause cutoffAt must be a valid timestamp.");
  }
